...
* `6.2`, `6.2.10`, `latest` - Splunk Enterprise
* `6.2-light`, `6.2.10-light`, `latest-light` - Splunk Light
* `6.2-forwarder`, `6.2.10-forwarder`, `latest-forwarder` - Splunk Universal Forwarder
...
```bash
docker pull outcoldman/splunk:6.2.10
```
...
```bash
docker run --hostname splunk -p 8000:8000 -d outcoldman/splunk:6.2.10
```
...
docker run --hostname splunk --name splunk --volumes-from=vsplunk -p 8000:8000 -d outcoldman/splunk:6.2.10
```
...
splunk:
  image: outcoldman/splunk:6.2.10
  hostname: splunk
  volumes_from:
    - vsplunk
  ports:
