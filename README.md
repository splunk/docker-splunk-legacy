...
* `6.2`, `6.2.11`, `latest` - Splunk Enterprise
* `6.2-light`, `6.2.11-light`, `latest-light` - Splunk Light
* `6.2-forwarder`, `6.2.11-forwarder`, `latest-forwarder` - Splunk Universal Forwarder

See other branches/tags if you need newer or older version of Splunk
...
```bash
docker pull outcoldman/splunk:6.2.11
```
...
```bash
docker run --hostname splunk -p 8000:8000 -d outcoldman/splunk:6.2.11
```
...
docker run --hostname splunk --name splunk --volumes-from=vsplunk -p 8000:8000 -d outcoldman/splunk:6.2.11
```
...
splunk:
  image: outcoldman/splunk:6.2.11
  hostname: splunk
  volumes_from:
    - vsplunk
  ports:
...
docker run --hostname splunk --name splunk --volumes-from=vsplunk -p 8000:8000 -d outcoldman/splunk:6.2.9
