...
* `6.5.1`, `latest` - Splunk universal forwarder base image [Dockerfile](https://github.com/splunk/docker-splunk/blob/master/enterprise/Dockerfile)
* `6.5.1-monitor` - Splunk universal forwarder with Docker Monitoring [Dockerfile](https://github.com/splunk/docker-itmonitoring/blob/master/Dockerfile)
...
0. Enter the following command to pull the Splunk Enterprise version 6.5.1 image.<br>
...
0. Run the Docker image.
   ...
   docker run --name splunkuniversalforwarder \
     ...
     --volume /var/log:/docker/log:ro \
       ...
       -d splunk/universalforwarder:6.5.1-monitor
   ```
...
The universal forwarder docker image can collect data from a host and send data to another host. If you pull the image with the '6.5.1-monitor` tag, the forwarder container includes the Docker data collection inputs.
...
The `6.5.1-monitor` tag ensures that the universal forwarder has the data inputs you need to get stats from a Docker container.

```bash
docker pull splunk/universalforwarder:6.5.1-monitor
```

### Pull the latest version of the image from this repository
The `6.5.1` and `latest` versions only have the forwarder and do not have any of the data inputs.
...
0. Insert the following block of text into the file.
   ...
   services:
    ...
    splunkuniversalforwarder:
     image: splunk/splunkuniversalforwarder:6.5.1-monitor
...
`splunk/universalforwarder:6.5.1` and `splunk/universalforwarder:latest`
This is the default universal forwarder image.

`splunk/universalforwarder:6.5.1-monitor`
